...
int main(int argc, char *argv[]) {
	...
	/*unsigned char ots_seed_g[10];

	for(int i=0;i<10;i++)
		ots_seed_g[i]= 0;

	for(int i=0;i<300;i++){
		increment_bytes(ots_seed_g, 10);

		for(int j=0;j<10;j++)
			printf("%hhu ",ots_seed_g[j]);
	...
	*/
	...
	if (argc >= 5) {
		...
		log_q = atoi(argv[3]);
		num_checkpoints = argc - 4;
		for (int i = 0; i < num_checkpoints; i++) {
			log_g_s[i] = atoi(argv[i + 4]);
		}
	} else {
		debug = 0;
		num_attack_iterations = 1;
	...
	printf("\tMemory usage (in bytes):\t%Lf\n", test_result.average_memory_usage / 8ll);
